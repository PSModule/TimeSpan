...
# Example 2: Get TimeSpan to future event
Write-Host "`n2. Getting TimeSpan to future events:" -ForegroundColor Yellow
$futureEvent = [DateTime]::Now.AddHours(3).AddMinutes(45)
$countdown = Get-TimeSpan -Target $futureEvent
...
# Example 3: Get TimeSpan for past events (age calculation)
Write-Host "`n3. Getting age/elapsed time:" -ForegroundColor Yellow
$pastEvent = [DateTime]::Now.AddDays(-30).AddHours(-5)
$age = Get-TimeSpan -Target $pastEvent -AsAge
Write-Host "Past event: $($pastEvent.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "Time since event: $($age.ToString())"
Write-Host "Formatted age: $(Format-TimeSpan $age -Precision 2 -FullNames)"

# Example 4: Handling negative values
Write-Host "`n4. Handling negative TimeSpan values:" -ForegroundColor Yellow
$pastDeadline = [DateTime]::Now.AddHours(-2)
$defaultResult = Get-TimeSpan -Target $pastDeadline
$allowNegativeResult = Get-TimeSpan -Target $pastDeadline -AllowNegative

Write-Host "Past deadline: $($pastDeadline.ToString('yyyy-MM-dd HH:mm:ss'))"
Write-Host "Default result (Zero): $($defaultResult.ToString())"
Write-Host "Allow negative result: $($allowNegativeResult.ToString())"
Write-Host "Formatted negative: $(Format-TimeSpan $allowNegativeResult -FullNames)"

# Example 5: Pipeline usage
Write-Host "`n5. Pipeline usage:" -ForegroundColor Yellow
$events = @(
